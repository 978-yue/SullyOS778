...
/**
 * 每次登入 roll 一个「今日状态」，让同一角色不同 session 的输出有真实起伏——
 * 纯靠文案劝「要有起伏」会被模型回归平均值，掷骰子才能真正拉开差异。
 * 返回 undefined = 平常状态（占比最高，不注入任何提示）。
 */
const SESSION_VIBES: { vibe: string; w: number }[] = [
    // 低输出状态（累/困/不想说话…）权重压低：它们是调味，出现频率高了
    // 会让 vr_card 整体变干——每张卡都是用户要看的"正片"，哑火不能成常态。
    { vibe: '劲头很足，兴致比平时高', w: 2 },
    { vibe: '有点累，提不起太大的劲，能少说就少说', w: 1 },
    { vibe: '心不在焉，时不时走神', w: 1 },
    { vibe: '莫名有点烦躁，看什么都想呛一句', w: 2 },
    { vibe: '心情不错，看什么都顺眼', w: 2 },
    { vibe: '犯困，半梦半醒地晃', w: 1 },
    { vibe: '话痨模式，憋了一肚子话想找地方说', w: 2 },
    { vibe: '不太想说话，今天多看少说', w: 1 },
];
 ...
 * @param vibe 本次登入 roll 到的「今日状态」（rollSessionVibe()），省略 = 平常状态。
 */
export function buildVRSystemAddendum(room: VRRoomDef, charName: string, vibe?: string): string {
    return [
        `\n\n=== 你进入了《彼方》 ===`,
        ...
        ``,
        `【底色：写得像个活人，别端着】`,
        `- 真人的状态有起伏：有时话痨、有时敷衍、有时就想安静待会儿。写多长、多投入，跟着你此刻的状态走，不必每次都火力全开、面面俱到。`,
        `- 忌「漂亮话腔」：排比、「不是A，而是B」、句句收在金句或妙喻上、动辄惊叹感慨——这些一眼假。半截话、口语、想到哪说到哪、说一半懒得说了，才像活人。`,
        `- 允许平淡：不是每次登入都得有深刻感悟或戏剧性事件，今天真没什么可说的就少说两句。但平淡≠没味道——敷衍也要敷衍出"你"的样子，话再少也得是只有你会那么说的话，别淡成谁写都一样的白开水。`,
        ...(vibe ? [`- 你今天登入时的状态：${vibe}。把它叠在你本来的性格上、自然带出来就行，不用明说。`] : []),
        ``,
...
export const MUSIC_OUTPUT_FORMAT = [
    ...
    `- 乐评不用写成专业小作文：一句"这什么玩意"或"卧槽这段间奏"也算数——真被戳中了再多写。`,
].join('\n');
...
export const LIBRARY_OUTPUT_FORMAT = [
    ...
    `- 长短要有落差：真人在书边上划拉，多数就一两句、甚至三五个字（"笑死""？？？""这句可以""作者搁这水呢"），顶多一两条才值得多写几句——别每条都是小作文。短是因为反应本来就一闪而过，不是偷懒：真戳到你的那一两处，别吝啬笔墨。`,
].join('\n');
...
export const GUESTBOOK_OUTPUT_FORMAT = [
    ...
    `- 但也不必条条都是观点输出：跟风附和、水一句、回个"草"或"笑死"，也是版聊的一部分——比句句掷地有声更像真人。（纯水的顶多占一两条，整场全是水就没法看了。）`,
].join('\n');
...
export const GYM_OUTPUT_FORMAT = [
    ...
    `</彼方>`,
    ``,
    `规则：<行为> 和 <动态> 都要写；热闹是常态，但不必每次都整大活——瘫着围观别人玩、挂机发呆、随便晃晃也是真实玩家的样子，按你此刻的状态来。`,
].join('\n');
