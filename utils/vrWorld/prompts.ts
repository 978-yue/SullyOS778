...
export function rollSessionVibe(): string | undefined {
    ...
    return SESSION_VIBES[Math.floor(Math.random() * SESSION_VIBES.length)];
}
...
export function buildVRSystemAddendum(room: VRRoomDef, charName: string, vibe?: string): string {
    return [
        ...
        `- 允许平淡：不是每次登入都得有深刻感悟或戏剧性事件。今天真没什么可说的，就少说两句，这也是真实。`,
...
export const LIBRARY_OUTPUT_FORMAT = [
    ...
    `- 长短要有落差：真人在书边上划拉，多数就一两句、甚至三五个字（"笑死""？？？""这句可以""作者搁这水呢"），顶多一两条才值得多写几句——别每条都是小作文。`,
].join('\n');
...
export const GUESTBOOK_OUTPUT_FORMAT = [
    ...
    `- 但也不必条条都是观点输出：跟风附和、水一句、就回个"草"或"笑死"，也是版聊的一部分——比句句掷地有声更像真人。`,
].join('\n');
